...
import com.xuxiaojian.aipassagecreator.service.strategy.ImageServiceStrategy;
...
/**
 * ClassName: ArticleAgentService
 * Package: com.xuxiaojian.aipassagecreator.service.impl
 * Description:
 *  智能体编排服务
 * @Author 阿健
 * @Create 2026-05-27 19:45
 * @Version 1.0
 */
@Service
@Slf4j
public class ArticleAgentService {
    ...
    public void executeArticleGeneration(ArticleState state, Consumer<String> streamHandler) {
        try {
            ...
            log.info("文档生成完成,taskId={}",state.getTaskId());

        }catch (Exception e) {
            log.error("文章生成失败,taskId={}",state.getTaskId(),e);
            throw new BusinessException(ErrorCode.OPERATION_ERROR,"文章生成失败: " + e.getMessage());
        }
    }
    ...
    public void mergeImagesIntoContent(ArticleState state) {
        ...
        log.info("图文合成成功,fullContentLength={}",fullContent.length());
    }
    ...
    public void agent5GenerateImages(ArticleState state, Consumer<String> streamHandler) {
        ...
        for (ArticleState.ImageRequirement requirement : state.getImageRequirements()) {
            ...
            log.info("智能体5:开始检索配图,position={},imageSource={},keywords={}",
            ...
            log.info("智能体5:配图检索成功,position={},method={},cosUrl={}",requirement.getPosition(),method.getValue(),cosUrl);
        }
    ...
    public void agent4AnalyzeImageRequirements(ArticleState state) {
        ...
        List<ArticleState.ImageRequirement> imageRequirements =
            parseJsonListResponse(content,new TypeToken<List<ArticleState.ImageRequirement>>(){},"配图需求");
    ...
    private String callLlmWithStreaming(String prompt, Consumer<String> streamHandler, SseMessageTypeEnum messageType) {
        ...
        .doOnError(error -> log.error("LLM 流式调用失败,messageType = {}",messageType.getDescription(),error))
        .blockLast();
    ...
    private ArticleState.ImageResult buildImageResult(ArticleState.ImageRequirement requirement, String imageUrl, ImageMethodEnum method) {
        ...
        imageResult.setPlaceholder(requirement.getPlaceholderId());
        return imageResult;
    }
    ...
    public void executePhase1_GenerateTitles(ArticleState state, Consumer<String> streamHandler) {
        try {
            ...
            throw new BusinessException(ErrorCode.SYSTEM_ERROR,"标题方案生成失败: " + e.getMessage());
        }
    }
    ...
    public void agent1GenerateTitleOptions(ArticleState state) {
        ...
        String content = callLlm(prompt);
        List<ArticleState.TitleOption> titleOptions = parseJsonListResponse(content,
     ...
     */
    public void execute2_GenerateOutline(ArticleState state, Consumer<String> streamHandler) {
        try {
            //智能体2：生成大纲（流式输出）
            ...
            throw new BusinessException(ErrorCode.SYSTEM_ERROR,"大纲生成失败: " + e.getMessage());
        }
    }
    ...
    public void executePhase3_GenerateContent(ArticleState state, Consumer<String> streamHandler) {
        try {
            ...
            getProxy().agent3GenerateContent(state,streamHandler);
            streamHandler.accept(SseMessageTypeEnum.AGENT3_COMPLETE.getValue());

            //智能体4:分析配图需求
            log.info("阶段3:开始分析配图需求,taskId={}",state.getTaskId());
            getProxy().agent4AnalyzeImageRequirements(state);
            streamHandler.accept(SseMessageTypeEnum.AGENT4_COMPLETE.getValue());

            //智能体5:生成配图
            log.info("阶段3:开始生成配图,taskId={}",state.getTaskId());
            getProxy().agent5GenerateImages(state,streamHandler);
            streamHandler.accept(SseMessageTypeEnum.AGENT5_COMPLETE.getValue());

            // 图文合成: 将配图插入正文
            log.info("阶段3:开始图文合成,taskId={}",state.getTaskId());
            getProxy().mergeImagesIntoContent(state);
            streamHandler.accept(SseMessageTypeEnum.MERGE_COMPLETE.getValue());

            log.info("阶段3,文档生成完成,taskId={}",state.getTaskId());
        }catch (Exception e) {
            log.error("阶段3：正文生成失败，taskId={}",state.getTaskId(),e);
            throw new BusinessException(ErrorCode.SYSTEM_ERROR,"正文生成失败: " + e.getMessage());
        }
    }
     ...
     * @param mainTitle
     * @param subTitle
     * @param currentOutline
     * @param modifySuggestion
     * @return
     */
    ...
    public List<ArticleState.OutlineSection> aiModifyOutline(String mainTitle, String subTitle, List<ArticleState.OutlineSection> currentOutline, String modifySuggestion) {
        ...
        ArticleState.OutlineResult outlineResult =
                parseJsonResponse(content,ArticleState.OutlineResult.class,"修改后的大纲");
